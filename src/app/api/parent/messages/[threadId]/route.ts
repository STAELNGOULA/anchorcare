...
export async function GET(_request: Request, context: RouteContext) {
  ...
  const thread = await getThreadForParent(user.id, threadId);
  if ("error" in thread) {
    const status = thread.error === "forbidden" ? 403 : 404;
    return NextResponse.json({ error: thread.error }, { status });
  }

  const result = await getThreadMessages(user.id, threadId, "parent");
  if ("error" in result) {
...
export async function POST(request: Request, context: RouteContext) {
  ...
  const result = await sendParentMessage(user.id, threadId, {
    body: body.body ?? "",
  });
  ...
  const refreshed = await getThreadMessages(user.id, threadId, "parent");
