...
type ParentUserMenuProps = {
  email: string;
};

export function ParentUserMenu({ email }: ParentUserMenuProps) {
        ...
        >
          ...
          <form action={signOutAction} className="border-t border-border/40 pt-1.5">
            <button
              ...
              role="menuitem"
              className={cn(
                "flex min-h-10 w-full items-center rounded-lg px-3 text-left text-sm text-muted-foreground transition-colors duration-300 ease-premium hover:bg-secondary/80 hover:text-foreground",
              )}
            >
              {t("userMenuSignOut")}
            </button>
