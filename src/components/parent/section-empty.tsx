...
export async function ParentSectionEmptyFromKey({
  namespace,
  ctaHref,
}: {
  namespace: "timeline" | "programs" | "care" | "profile";
  ctaHref?: string;
}) {
  const t = await getTranslations(`parent.${namespace}`);
  const ctaLabel = namespace === "programs" ? t("emptyCta") : undefined;
