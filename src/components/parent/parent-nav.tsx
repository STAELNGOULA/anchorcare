...
import { isParentNavActive, PARENT_NAV } from "@/lib/parent/navigation";
import { parentRailWidth } from "@/lib/parent/layout";
import {
  shellNavHeight,
  shellNavStickyTop,
    ...
    >
      <div className="flex h-full flex-col rounded-[1.5rem] border border-border/50 bg-card/90 p-2 shadow-soft backdrop-blur-xl">
        <ul className="space-y-1">
          {PARENT_NAV.map((item) => {
            const active = isParentNavActive(pathname, item.href);
