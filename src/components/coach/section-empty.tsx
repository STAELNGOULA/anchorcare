...
type CoachEmptyNamespace = "report" | "roster" | "incidents";

export async function CoachSectionEmptyFromKey({
  namespace,
  ctaHref,
}: {
  namespace: CoachEmptyNamespace;
  ctaHref?: string;
}) {
  const t = await getTranslations(`coach.${namespace}`);
  const ctaLabel =
    namespace === "report" ? t("emptyCta") : undefined;
